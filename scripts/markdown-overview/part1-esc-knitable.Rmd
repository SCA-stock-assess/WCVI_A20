...
```{r}
esc_ts <- sj.esc %>% 
  filter(est_type %in% c("Natural Adult Spawners", "Adult Broodstock Removals"), 
...
esc_avgsLT <- esc_ts %>%
  filter(year >= 1995) %>% 
  group_by(Species) %>% 
  summarize(mean_esc_full = mean(estimate)) %>%
  print()

esc_avgs10 <- esc_ts %>%
  filter(year %in% c(analysis_year-1:(analysis_year-1-10))) %>% 
  group_by(Species) %>% 
  summarize(mean_esc_full = mean(estimate)) %>%
  print()
...
esc_quart <- esc_ts %>%
  ...
  group_by(Species) %>%
  summarize(
    Q0 = quantile(estimate, probs = 0),
    Q1 = quantile(estimate, probs = 0.25),
    Median = quantile(estimate, probs = 0.5),
    Q3 = quantile(estimate, probs = 0.75),
    Q4 = quantile(estimate, probs = 1)
  ) %>%
  print()

esc_quart_CN <- quantile(esc_ts[esc_ts$Species=="Chinook" & esc_ts$year >= 1995,]$estimate)
```
