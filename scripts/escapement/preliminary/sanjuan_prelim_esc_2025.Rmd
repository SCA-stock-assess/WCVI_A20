...
Mini tribs were surveyed this year and mostly entered under San Juan mainstem as reaches. Three are not in the database and need to be added next year: Dr Dave Creek, Chim's Channel, and Stanley Creek. These are all part of the San Juan mainstem estimate, but are not of concern because 1) all with the exception of Kuzman are d/s of the standard snorkel area, and 2) hardly any fish were seen in 2025 in these creeks. 
...
**In absence of any other information, we will assume 66% of the total number went into Harris Creek, or 5908*0.66 = 3,899 live adults + 1 dead = 3,900. Subtracting 5908-3900 = 2,008 live coho + 1 dead = 2,009 coho spawning in the San Juan mainstem. Likewise for jacks this would equate to 58 total (expanded) jacks, 39 in Harris and 19 in the SJ mainstem.**
...
ggplot()  +
  geom_bar(data=a20_nuseds_escapement %>% 
             filter(grepl("SAN JUAN|harris|lens",waterbody_name, ignore.case=T), `Species Qualified`=="CO", 
                    est_type%in%c("Unspecified Return", "Natural Adult Spawners")) %>%
             group_by(year, est_type) %>%
             summarize(count = sum(estimate, na.rm=T)), 
           aes(x=year, y=count, colour=est_type, fill=est_type), stat="identity", alpha=0.8) +
   # geom_bar(data=escEstSpp %>% 
   #            filter(ShortSpecies=="CO", grepl("San Juan", SystemName)) %>% 
   #            select(Year, SystemName, AdultNaturalReco) %>% 
   #            summarize(n=sum(AdultNaturalReco)), 
   #          aes(x=analysis_year, y=n), stat="identity", colour="red", fill="dodger blue", alpha=0.4) +   
     geom_bar(aes(x=2025, y=5903), stat="identity", colour="red", fill="dodger blue", alpha=0.4) + 
  scale_fill_manual(values=c("Unspecified Return" = "gray80",
                             "Natural Adult Spawners" = "dodger blue")) +
  scale_colour_manual(values=c("Unspecified Return" = "gray80",
                               "Natural Adult Spawners" = "dodger blue")) +
...
Used expanded PL+D. Added in 4 dead chum in Kuzman as not likely these would have been double-counted (43 expanded live + 4 dead = 47 total).
